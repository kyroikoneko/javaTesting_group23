...
public class ContactDataGenerator {
	...
	private static void saveContactsToCsvFile(List<ContactData> contacts, File file) throws IOException {
		...
		for (ContactData contact : contacts) {
			writer.write(contact.getSecondName() + "," + contact.getFirstName() + "," + contact.getPostAddress() + "," 
		+ contact.getHomePhoneNum() + "," + contact.getMobilePhonNum() + "," + contact.getWorkPhoneNum() + "," 
					+ contact.getPostPrimary() + "," + contact.getPostSecondary() + "," + contact.getSecondaryPostAddress() + "," 
	...
	public static List<ContactData> generateRandomContacts(int amount) {
		  ...
		  for (int i = 0; i < amount; i++) {
			...
			.withPostSecondary(emailRandomizer())
			.withDay("18")
			.withMonth("January")
			.withYear("1992")
